...
FilterBar :: createActivityCombo( )
{
    ...
    row = new QStandardItem( QtIconLoader::icon( "system-run", blankIcon ), tr( "Active" ) );
    row->setData( FilterMode::SHOW_ACTIVE, ActivityRole );
    model->appendRow( row );

    row = new QStandardItem( QtIconLoader::icon( "go-down", blankIcon ), tr( "Downloading" ) );
    row->setData( FilterMode::SHOW_DOWNLOADING, ActivityRole );
    model->appendRow( row );

    row = new QStandardItem( QtIconLoader::icon( "go-up", blankIcon ), tr( "Seeding" ) );
    ...
    myIsBootstrapping( true )
{
    ...
    QIcon icon = QtIconLoader::icon( "edit-clear", style()->standardIcon( QStyle::SP_DialogCloseButton ) );
